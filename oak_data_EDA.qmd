...
# seedling height, # of leaves, and acorn germination for Winter 2025
winter25 <- read_csv('2025 Winter Seedling & Acorn Height and Survival ORPOY - Sheet1.csv')

#making the cloumn names match the preexisting data sets 
winter25 <- winter25 |>
  select(-`Data Enterer Initials`, -`Page Number`, -`Date`, -`Data Entered`, -`QAQC`, -`Notes`) |>
  rename('Block' = 'Block Number') |>
  rename('Tag' = 'Tag Number') |>
  rename('LifeStage' = 'Acorn or seedling?') |>
  rename('Height' = 'Seedling Height (cm)') |>
...
#Winter 2025 soil moisture data
winter25SM <- read_csv('2025 Winter Soil Moisture ORPOY - Sheet1.csv')
...
winter25_summary <- winter25 %>%
  group_by(Block, Treatment) %>%
  summarise(Total_Germinated = sum(GerminatedAcorns, na.rm = TRUE))

# Calculate total germinated acorns across all blocks
total_germinated <- sum(winter25_summary$Total_Germinated, na.rm = TRUE)

# Get the maximum value for y-axis scaling
max_value <- max(winter25_summary$Total_Germinated, na.rm = TRUE)

# Creating the faceted plot with a total count in the caption
ggplot(winter25_summary, aes(x = Treatment, y = Total_Germinated, fill = Treatment)) +
  geom_bar(stat = "identity") +
  facet_grid(~Block) +  # Facet by Block
  labs(
    title = "Number of Germinated Acorns by Treatment and Block",
    x = "Treatment",
    y = "Total Germinated Acorns",
    fill = "Treatment",
    caption = paste("Total germinated acorns across all blocks:", total_germinated, "\n",
      "Out of: 288")  # Add text under legend
  ) +
...
winter25_heights <- winter25 |>
filter(LifeStage == 'Seedling') |> 
  select(Tag, Treatment, Height, Block) |> 
  mutate(Season = 'Winter25') |>
  mutate(Lifestage = 'Seedling')

# Combine the  datasets
seedling_height_data <- bind_rows(initial_heights, spring_heights, summer_heights, fall_heights, winter25_heights)

# View the combined data
print(seedling_height_data)

seedling_height_data <- seedling_height_data |>
  mutate(Season = factor(Season, levels = c("Initial24", "Spring24", "Summer24", "Fall24", "Winter25")))
...
growth_rate_data <- seedling_height_wide |>
  select(Tag, Treatment, Block, Growth_Initial_to_Spring, Growth_Spring_to_Summer, Growth_Summer_to_Fall, Growth_Fall_to_Winter) |>
  pivot_longer(
...
winter25SM$Treatment <- as.factor(winter25SM$Treatment) 
winter25SM$Block <- as.factor(winter25SM$Block)
...
winter25SM <- winter25SM |>
  mutate(Season = "Winter 2025")
...
combinedSM_data <- merge(combinedSM_data, winter25SM, by = c("Treatment", "Block"))
...
ggplot(combinedSM_data, aes(x = MeanVWC, y = Growth_Rate)) +
  geom_point(aes(color = Treatment), size = 3) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  labs(
    x = "Mean Volumetric Water Content",
...
model <- lm(Growth_Rate ~ MeanVWC, data = combinedSM_data)
summary(model)
...
spring_leaves <- spring24 |> 
  filter(LifeStage == 'Seedling', Treatment != 'NR', Height > 1, Leaves > 1) |> 
...
# Filter the winter25 leaf count data
winter_leaves <- winter25 |> 
  filter(LifeStage == 'Seedling', Treatment != 'NR', Height > 1, Leaves > 1) |> 
  select(Tag, Treatment, Leaves, Block) |> 
  mutate(Season = 'Winter25') |>
  mutate(Lifestage = 'Seedling')
...
# Combine the  datasets
seedling_leaf_data <- bind_rows(spring_leaves, summer_leaves, fall_leaves, winter_leaves)
...
winter25SM <- winter25SM |>
  mutate(across(c(PER1, PER2, PER3), ~ as.numeric(.)))

# Combine the datasets
combinedSM_data <- bind_rows(springSM, fallSM, winter25SM)
...
germination_data <- bind_rows(
  spring24 |> select(Block, Treatment, GerminatedAcorns) |> mutate(Season = "Spring 24"),
  summer24 |> select(Block, Treatment, GerminatedAcorns) |> mutate(Season = "Summer 24"),
  fall24 |> select(Block, Treatment, GerminatedAcorns) |> mutate(Season = "Fall 24"),
  winter25 |> select(Block, Treatment, GerminatedAcorns) |> mutate(Season = "Winter 25")
)

#change the order of the seasons to match daata collection dates 
germination_data <- germination_data |>
  mutate(Season = factor(Season, levels = c("Spring 24", "Summer 24", "Fall 24", "Winter 25")))
...
germination_summary |>
  ...
  geom_col() +
  geom_text(aes(label = Total_Germinated), vjust = -0.5) +
  labs(title = "Total Germinated Seeds by Season", 
       x = "Season", 
       y = "Total Germinated")
```
